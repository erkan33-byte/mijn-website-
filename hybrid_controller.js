...
(() => {
  ...
  function readConfig() {
    ...
    return {
      ...
      gpuKeys, cpuKeys,
      gpuDepth: Math.max(1, Math.min(4, Number(h('gpuPipelineDepth').value) || 2)),
  ...
  function assignCpuLease(id, token) {
    ...
    renderLeases();
    const worker = cpuWorkers[id];
    if (!worker) { release(label); h('cpuStatus').value = `CPU-worker ${id + 1} ontbreekt`; return; }
  ...
  function startCpuWorker(id, token) {
    if (!isRunning || token !== hybridToken) return;
    const label = 'cpu:' + id;
    const worker = new Worker('worker_native_hybrid.js?build=20260821');
    cpuWorkers[id] = worker;
    worker.onmessage = ({data}) => {
      if (!isRunning || token !== hybridToken) return;
      if (data.type === 'progress') { cpuScanned += Number(data.count); updateHybridCounters(); }
      else if (data.type === 'done') { release(label); cpuCompleted++; updateHybridCounters(); assignCpuLease(id, token); }
      else if (data.type === 'found') { cpuScanned += Number(data.count); updateHybridCounters(); reportMatch(data.key, 'native WASM'); }
      else if (data.type === 'error') {
        release(label); worker.terminate(); cpuWorkers[id] = null;
        log(`CPU-worker ${id + 1} fout: ${data.error}`, 'warn'); h('cpuStatus').value = `CPU-worker ${id + 1} fout`;
      }
    };
    worker.onerror = e => {
      if (!isRunning || token !== hybridToken) return;
      release(label); worker.terminate(); cpuWorkers[id] = null;
      log(`CPU-worker ${id + 1} crash: ${e.message}`, 'warn'); h('cpuStatus').value = `CPU-worker ${id + 1} crash`;
    };
    assignCpuLease(id, token);
  }
  ...
  async function gpuLoop(token) {
    ...
    while (isRunning && token === hybridToken) {
      ...
      const stage = pipelineStages[stageIndex]; stageIndex=(stageIndex+1)%cfg.gpuDepth;
  ...
  startBtn.addEventListener('click', async () => {
    try {
      ...
      log(`Hybride start: GPU-leases=${cfg.gpuKeys.toString()}, GPU-pipeline=${cfg.gpuDepth} stage(s), CPU=${cfg.cpuCount} × ${cfg.cpuKeys.toString()} keys.`, 'hop');
